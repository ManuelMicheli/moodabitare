    ...
    <div className="fixed inset-0 z-[100]">
      ...
      <div className="absolute inset-0 flex items-end lg:hidden pointer-events-none">
        ...
        >
          ...
          >
            <div className="relative aspect-[4/3] bg-gradient-to-b from-warm-gray/50 to-white mx-6 mt-2">
              <Image
                src={imageSrc}
                alt={detail.name}
                fill
            ...
            <div className="px-6 pb-4 pt-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-label text-black-deep/70 mt-1">{detail.brand}</p>
                </div>
                <span
                  className={`flex-shrink-0 text-[0.7rem] font-ui font-semibold uppercase tracking-[0.12em] px-2.5 py-1 rounded-full border ${segmentStyle[detail.segment] || "border-black-deep/10 text-black-deep/70"}`}
                >
                  {detail.segment}
                </span>
              </div>
              ...
              <p className="mt-3 text-[0.95rem] font-display text-black-deep leading-relaxed">
                {detail.description}
              </p>

              <div className="mt-6">
                <p className="text-label text-black-deep/70 mb-3">Specifiche tecniche</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                  {detail.specs.map((s) => (
                    <div key={s.label} className="bg-warm-gray/20 px-3 py-2.5 rounded-md">
                      <span className="text-[0.75rem] uppercase tracking-wider text-black-deep/60">{s.label}</span>
                      <span className="text-sm text-black-deep block mt-0.5">{s.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-5 grid grid-cols-1 gap-3">
                <div className="py-2.5 border-b border-black-deep/5">
                  <span className="text-[0.75rem] font-ui text-black-deep/60 uppercase tracking-wide">Finiture disponibili</span>
                  <p className="text-sm text-black-deep mt-0.5">{detail.colors}</p>
                </div>
                <div className="py-2.5">
                  <span className="text-[0.75rem] font-ui text-black-deep/60 uppercase tracking-wide">Caratteristiche</span>
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {detail.features.map((feat) => (
                      <span key={feat} className="text-[0.75rem] font-ui text-black-deep bg-warm-gray/40 px-2 py-0.5 rounded">
                        {feat}
                      </span>
                    ))}
                  </div>
          ...
          <div
            className="border-t border-black-deep/5 px-6 py-4 bg-white flex gap-3"
            style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom, 0px))" }}
          >
            <a href="/contatti" className="flex-1 text-button text-center bg-black-deep text-white py-3.5 hover:bg-black-soft transition-colors">
              Richiedi preventivo
            </a>
            <a
              ...
              >
                ...
                <motion.div variants={fadeUp}>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="font-ui text-[0.7rem] uppercase tracking-[0.2em] text-black-deep/60">
                      {detail.brand}
                    </span>
                ...
                <motion.div variants={fadeUp}>
                  <p className="font-ui text-[0.65rem] uppercase tracking-[0.2em] text-black-deep/60 mb-5">
                    Specifiche tecniche
                  </p>
                  <div className="space-y-0">
                    {detail.specs.map((s, i) => (
                      ...
                      >
                        <span className="font-ui text-[0.8rem] text-black-deep/60 uppercase tracking-[0.06em]">
                          {s.label}
                        </span>
                ...
                <motion.div variants={fadeUp} className="grid grid-cols-2 gap-10">
                  <div>
                    <p className="font-ui text-[0.65rem] uppercase tracking-[0.2em] text-black-deep/60 mb-3">
                      Finiture
                    </p>
