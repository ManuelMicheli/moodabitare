...
function SheetContent({ detail, imageSrc, onClose }: { detail: ProductDetail; imageSrc: string; onClose: () => void }) {
  ...
  // Block scroll on background — prevent touchmove on backdrop only
  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="absolute inset-0 bg-black-deep/60 backdrop-blur-sm"
        onClick={onClose}
      ...
      <div className="absolute inset-0 flex items-end sm:items-center sm:justify-center pointer-events-none">
        <motion.div
          ...
          className="pointer-events-auto w-full sm:max-w-lg lg:max-w-xl bg-white sm:rounded-2xl rounded-t-2xl relative"
        ...
        >
            ...
            <div className="px-6 pb-4 pt-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-label text-mid-gray mt-1">{detail.brand}</p>
                </div>
                <span
              ...
              <div className="mt-6">
                <p className="text-label text-mid-gray mb-3">Specifiche tecniche</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                  {detail.specs.map((s) => (
                    <div key={s.label} className="bg-warm-gray/20 px-3 py-2.5 rounded-md">
                      <span className="text-[0.75rem] uppercase tracking-wider text-mid-gray">{s.label}</span>
              ...
              <div className="mt-5 grid grid-cols-1 gap-3">
                <div className="py-2.5 border-b border-black-deep/5">
                  <span className="text-[0.75rem] font-ui text-mid-gray uppercase tracking-wide">Finiture disponibili</span>
                  <p className="text-sm text-black-deep mt-0.5">{detail.colors}</p>
                </div>
                <div className="py-2.5">
                  <span className="text-[0.75rem] font-ui text-mid-gray uppercase tracking-wide">Caratteristiche</span>
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {detail.features.map((feat) => (
                      <span key={feat} className="text-[0.75rem] font-ui text-mid-gray bg-warm-gray/40 px-2 py-0.5 rounded">
                        {feat}
                      </span>
                    ))}
                  </div>
          ...
          {/* CTA */}
          <div className="border-t border-black-deep/5 px-6 py-4 bg-white sm:rounded-b-2xl flex gap-3" style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom, 0px))" }}>
